...
def test_rozdej_sloupecky_nechybi_karty():
    ...
    from klondike import vytvor_balicek, rozdej_sloupecky, otoc_kartu
    balicek = vytvor_balicek()
    kopie_puvodniho_balicku = list(balicek)
    sloupecky = rozdej_sloupecky(balicek)
...
def test_vypis_prazdne_sloupecky(capsys):
    from klondike import vypis_sloupecky
    vypis_sloupecky({'A': [], 'B': [], 'C': [], 'D': [], 'E': [], 'F': [], 'G': [], 'H': []})
...
def test_vypis_sloupecky_dvou_kartach(capsys):
    from klondike import vypis_sloupecky
    vypis_sloupecky({
        'A': [(1, 'Pi', True), (7, 'Sr', True)],
        'B': [(2, 'Sr', True), (6, 'Ka', True)],
        'C': [(3, 'Ka', True), (5, 'Kr', False)],
        'D': [(4, 'Kr', False), (4, 'Pi', True)],
        'E': [(5, 'Pi', False), (3, 'Sr', True)],
        'F': [(6, 'Sr', True), (2, 'Ka', True)],
        'G': [(7, 'Ka', True), (1, 'Kr', True)],
    })
    out, err = capsys.readouterr()
    check_text(out, """
        [A♠ ] [2 ♥] [3 ♦] [???] [???] [6 ♥] [7 ♦]
...
def test_vypis_sloupecky_vice_karet(capsys):
    ...
    vypis_sloupecky({
        'A': [(1, 'Pi', True)],
        'B': [(2, 'Pi', True), (2, 'Sr', True)],
        'C': [(3, 'Pi', True), (3, 'Sr', True), (3, 'Ka', True)],
        'D': [(4, 'Pi', True), (4, 'Sr', True), (4, 'Ka', False), (4, 'Kr', True)],
        'E': [(5, 'Pi', True), (5, 'Sr', True), (5, 'Ka', True)],
        'F': [(6, 'Pi', True), (6, 'Sr', True)],
        'G': [(7, 'Pi', True)],
    })
    out, err = capsys.readouterr()
    check_text(out, """
